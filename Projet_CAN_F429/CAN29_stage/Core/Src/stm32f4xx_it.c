                ...
                case module_ADC_DMA_control_ID:  // Module ID for ADC DMA
                    switch (Rx_Data[1])  // Routine ID = Rx_Data[1]
                    {
                        case ADC_start:
                            ...
                            {
                            	HAL_GPIO_TogglePin(GPIOG, GPIO_PIN_14); //to check timer
                            }
                            Send_CAN_Message(Rx_Data, 0x8U);  // Send response
                            break;
                        ...
                        case ADC_get_values:
                        {
                            ...
                            HAL_GPIO_TogglePin(GPIOG, GPIO_PIN_13); //to check timer
                            Send_CAN_Message(Rx_Data, 0x8U);
                            break;
                        }
                        break;
...
void TIM4_IRQHandler(void)
{
  ...
  PWM_Measurement pwm = Handle_TIM_IC_Capture( TIM_CHANNEL_1);
